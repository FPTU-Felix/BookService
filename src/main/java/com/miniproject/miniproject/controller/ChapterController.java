...
@RestController
@RequestMapping("/api/v1/library/chapter")
...
public class ChapterController {
    ...
    public ResponseEntity<ApiResponse<ChapterDetailResponse>> getChapterById(@PathVariable String chapterId) {
        ApiResponse<ChapterDetailResponse> response = chapterService.getChapterById(chapterId);
        return ResponseEntity.ok(response);
    }

    @PostMapping("/addChapter")
    public ResponseEntity<ApiResponse<ChapterResponse>> addChapter(@RequestBody @Valid ChapterRequest request,
            @PathVariable String previousChapterId, Authentication authentication) {
        String userId = authentication.getName();
        ChapterResponse chapterResponse = chapterService.addChapter(request, userId, previousChapterId);
        ApiResponse<ChapterResponse> response = new ApiResponse<>(String.valueOf(HttpStatus.CREATED), chapterResponse,
                null);
        return new ResponseEntity<>(response, HttpStatus.CREATED);
    }

    @PutMapping("/{postId}")
    public ResponseEntity<ApiResponse<ChapterResponse>> updateChapter(@RequestBody @Valid ChapterRequest request,
